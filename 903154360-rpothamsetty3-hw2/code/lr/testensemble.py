...
def predict_prob(classifiers, X):
    ...
    for model in classifiers:
      ensemble_pred.append(model.predict_prob(X))
    return sum(ensemble_pred)/float(len(ensemble_pred))


if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option("-m", "--model-path", action="store", dest="path",
                      default="models6", help="path where trained classifiers are saved")
    parser.add_option("-r", "--result", action="store", dest="result",
                      default="rocens_01_05", help="name of the figure")
